...
public class SymptomsDistributionFragment extends Fragment {

    private Context mContext;
    ListView mSymptomsDistribution;
    ArrayList<String> mSymptomsDistributionList;
    DCAApplication dcaApplication;
    TableLayout tblLaySymptomDistribution;

    public SymptomsDistributionFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fragment_symptoms_distribution, container, false);

        mSymptomsDistribution = rootView.findViewById(R.id.lvSymptomsDistribution);
        mSymptomsDistributionList = new ArrayList<>();
        tblLaySymptomDistribution = rootView.findViewById(R.id.tblSymptomDistribution);
                    ...
                    switch (section.getFields().get(i).getFieldType()) {
                        case 9: {
                            if (section.getFields().get(i).isMultiValued()) {
                                TextView tvBaseTitleSymptomsDistribution = rootView.findViewById(R.id.baseTitleSymptomsDistribution);
                                tvBaseTitleSymptomsDistribution.setText(ApiData.getInstance().getMetadataTranslationsList().get(section.getFields().get(i).getTranslationId()).getValue());

                                if (tblLaySymptomDistribution.getChildCount() > 0) {
                                    tblLaySymptomDistribution.removeAllViews();
                                }
                                ...
                                if (section.getFields().get(i).getOptions() != null && section.getFields().get(i).getOptions().size() > 0) {
                                    for (int j = 0; j < section.getFields().get(i).getOptions().size(); j++) {
                                        ...
                                        cb.setId(9400 + i);
                                        tr.addView(relLay);
                                        tblLaySymptomDistribution.addView(tr);
                                        mSymptomsDistributionList.add(val);
                                    }
                                }

                                if (ApiData.getInstance().getCurrentForm() != null) {
                                    if (!TextUtils.isEmpty(ApiData.getInstance().getCurrentForm().getSymptomsDistribution())) {
                                        String[] strSelSymDistribution = ApiData.getInstance().getCurrentForm().getSymptomsDistribution().split("##");
                                        if (strSelSymDistribution != null && strSelSymDistribution.length > 0) {
                                            for (int k = 0; k < tblLaySymptomDistribution.getChildCount(); k++) {
                                                RelativeLayout relLay = (RelativeLayout) ((TableRow) tblLaySymptomDistribution.getChildAt(k)).getChildAt(0);
                                                CheckBox chkBox = (CheckBox) relLay.findViewWithTag(mSymptomsDistributionList.get(k));
                                                for (int l = 0; l < strSelSymDistribution.length; l++) {
                                                    if (chkBox.getText().toString().toUpperCase().equals(strSelSymDistribution[l].toUpperCase())) {
                        ...
                        break;
                        case 10: {
                            LinearLayout baseLayout = rootView.findViewById(R.id.baseLayoutSymptomsDistribution);
                                ...
                                public void onClick(View view) {
                                    if (sectionPos > -1) {
                                        if (ApiData.getInstance().getCurrentForm() != null) {
                                            ...
                                            for (int k = 0; k < tblLaySymptomDistribution.getChildCount(); k++) {
                                                CheckBox chkBox = ((CheckBox) ((RelativeLayout) ((TableRow) tblLaySymptomDistribution.getChildAt(k)).getChildAt(0)).getChildAt(0));
                                                if (chkBox.isChecked()) {
                                                    selectedItems = selectedItems + chkBox.getText() + "##";
                                                }
                                            }
                                            ApiData.getInstance().getCurrentForm().setSymptomsDistribution(selectedItems);
                                        }
                                        if (fromSummary) {
